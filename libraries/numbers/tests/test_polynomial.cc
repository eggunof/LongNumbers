...
TEST(PolynomialTest, Derivative) {
  ...
  EXPECT_EQ(pol1.GetLeadingCoefficient(), pol2.GetLeadingCoefficient());
  EXPECT_EQ(pol1, pol2);
  ...
  EXPECT_EQ(pol3.GetLeadingCoefficient(), pol4.GetLeadingCoefficient());
  EXPECT_EQ(pol3, pol4);
  ...
  EXPECT_EQ(pol5.GetLeadingCoefficient(), pol6.GetLeadingCoefficient());
  EXPECT_EQ(pol5, pol6);

  Polynomial pol7({{Natural(10), Rational(Integer(264), Natural(1))}});
  pol5 = pol5.Derivative(pol5);
  EXPECT_EQ(pol5.GetDegree(), pol7.GetDegree());
  EXPECT_EQ(pol5.GetLeadingCoefficient(), pol7.GetLeadingCoefficient());
  EXPECT_EQ(pol5, pol7);
}
