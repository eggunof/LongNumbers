...
Polynomial Polynomial::GreatestCommonDivisor(const Polynomial &first,
  ...
  if (first.GetDegree() >=
      second
          .GetDegree()) {  // если степень первого превосходи или равна второму
  ...
  remains =
      dividend %
      divisor;  // первый раз находим остаток от деления первого на второго
  while (!remains.coefficients_
              .empty()) {  // пока отсаток не равен 0 (не пуст равнозначно)
